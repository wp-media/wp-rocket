...
- Sensitive data written to disk under a web-accessible path (anything under `wp-content/`, including plugin-created directories): settings backups, exports, logs, or cache artifacts containing API keys, tokens, license data, or PII must not be directly fetchable over HTTP. `.htaccess deny` alone is insufficient — it does nothing on Nginx. An `index.php` stub in the directory is **also not a valid fix** — it only intercepts requests for the directory index, not a direct request for a sibling static file (e.g. `backup_X.json`), which Apache and Nginx both serve straight from disk without ever invoking PHP. Require at least one of: storing the data outside the web-served tree entirely, or serving its contents only through an authenticated PHP handler (e.g. an admin-ajax/REST callback gated by `current_user_can()`) instead of letting the web server serve the raw file — or encrypting/redacting the sensitive fields before they touch disk. A guessable or brute-forceable filename (version numbers, timestamps, sequential IDs) is not a mitigation on its own — flag it even if some access-block exists, unless the spec/tests demonstrate the block works on both Apache and Nginx.
