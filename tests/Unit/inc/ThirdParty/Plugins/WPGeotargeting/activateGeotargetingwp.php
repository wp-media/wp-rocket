...
class Test_activateGeotargetingwp extends TestCase {
    ...
    {
		Filters\expectAdded('rocket_htaccess_mod_rewrite')->with('__return_false', 72);
		Filters\expectAdded('rocket_cache_dynamic_cookies')->with([$this->wpgeotargeting, 'add_geotargetingwp_dynamic_cookies']);
		Filters\expectAdded('rocket_cache_mandatory_cookies')->with([$this->wpgeotargeting, 'add_geotargetingwp_mandatory_cookie']);
