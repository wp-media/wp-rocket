 ...
 * @group Preload
 */
class TestProcessPendingJobs extends TestCase {
	protected $queue;
	protected $query;
	protected $options;
	...
	public function testShouldDoAsExpected( $config, $expected ) {
		...
		$this->queue->expects()->get_pending_preload_actions()
			->andReturn( [] );

		Filters\expectApplied( 'rocket_preload_cache_pending_jobs_cron_rows_count' )
			->with( 100 )
			->andReturn( $config['rows'] );

		$this->query->expects( self::once() )
			->method( 'get_outdated_in_progress_jobs' )
			->with()
		...
		$this->query->expects( self::atLeast( 0 ) )
			...
			->withConsecutive( $expected['job_deleted'] );

		$this->controller->shouldReceive( 'is_excluded_by_filter' )
			->zeroOrMoreTimes()
			->andReturnValues( $config['excluded'] );

		foreach ( $expected['job_urls'] as $url ) {
			$this->queue->expects()
			->add_job_preload_job_preload_url_async( $url );
		}
