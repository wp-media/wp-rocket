...
/**
 * @covers \WP_Rocket\Engine\Optimization\DynamicLists::get_js_minify_excluded_external
 *
 * @group  DynamicLists
 */
class Test_GetJsMinifyExcludedExternal extends TestCase {
	/**
	 * @dataProvider configTestData
	 */
	public function testShouldReturnExpected( $list, $expected ) {
		$data_manager  = Mockery::mock( DataManager::class );
		$dynamic_lists = new DynamicLists( Mockery::mock( APIClient::class ), $data_manager, '' );
		...
		$this->assertSame(
			$expected,
			$dynamic_lists->get_js_minify_excluded_external()
		);
	}
}
