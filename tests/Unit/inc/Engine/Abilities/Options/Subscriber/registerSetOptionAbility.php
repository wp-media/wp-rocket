...
/**
 * Tests for WP_Rocket\Engine\Abilities\Options\Subscriber::register_set_option_ability()
 *
 * @group Abilities
 */
class Test_RegisterSetOptionAbility extends TestCase {
	/**
	 * Test that register_set_option_ability() honors the abilities gate.
	 *
	...
	public function testShouldReturnExpected( array $config, array $expected ): void {
		...
		if ( $expected['register_called'] ) {
			$set_option->shouldReceive( 'register' )->once();
		} else {
			$set_option->shouldReceive( 'register' )->never();
		}

		$subscriber = new Subscriber( $get_options, $set_option, $context );
		$subscriber->register_set_option_ability();
	}
}
