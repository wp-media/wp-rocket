...
class Test_CleanDomain extends TestCase
{
    ...
    public function testShouldDoExpected($config, $expected)
    {
        ...
        if ($expected) {
            Functions\expect('wp_remote_request')->once()->with('http://example.org/.*', ['method' => 'PURGE', 'blocking' => false, 'redirection' => 0, 'headers' => ['host' => 'example.org', 'X-Purge-Method' => 'regex']]);
        } else {
            Functions\expect('wp_remote_request')->never();
        }
        do_action('before_rocket_clean_domain', 'wp-rocket/cache', '', 'http://example.org');
    }
