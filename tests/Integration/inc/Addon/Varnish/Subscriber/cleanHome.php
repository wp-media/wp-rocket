...
class Test_CleanHome extends TestCase
{
    ...
    public function testShouldDoExpected($config, $expected)
    {
        ...
        if ($expected) {
            Functions\expect('wp_remote_request')->once()->with('http://example.org/', ['method' => 'PURGE', 'blocking' => false, 'redirection' => 0, 'headers' => ['host' => 'example.org', 'X-Purge-Method' => 'default']])->andAlsoExpectIt()->once()->with('http://example.org/page/.*', ['method' => 'PURGE', 'blocking' => false, 'redirection' => 0, 'headers' => ['host' => 'example.org', 'X-Purge-Method' => 'regex']]);
        } else {
