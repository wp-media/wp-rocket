...
class Test_CleanFile extends TestCase
{
    ...
    public function testShouldDoExpected($config, $expected)
    {
        ...
        if ($expected) {
            Functions\expect('wp_remote_request')->once()->with('http://example.org/about/.*', ['method' => 'PURGE', 'blocking' => false, 'redirection' => 0, 'headers' => ['host' => 'example.org', 'X-Purge-Method' => 'regex']]);
        } else {
