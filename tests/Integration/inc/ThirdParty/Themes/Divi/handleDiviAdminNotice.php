...
class Test_HandleDiviAdminNotice extends WPThemeTestcase
{
    ...
    public function testAdminNotice($config, $expected)
    {
        ...
        if ($expected['notice_show'] && $expected['notice_html']) {
            Functions\expect('wp_create_nonce')->twice()->andReturn('12345');
            $this->assertStringContainsStringIgnoringCase($this->format_the_html($expected['notice_html']), $this->getActualHtml());
        } else {
