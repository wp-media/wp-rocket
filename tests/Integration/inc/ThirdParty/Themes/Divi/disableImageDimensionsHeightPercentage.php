...
class Test_DisableImageDimensionsHeightPercentage extends WPThemeTestcase
{
    ...
    public function testRemovesCorrectImagesFromAddDimensionsArray($config, $expected, $html)
    {
        ...
        if (!$config['is-child']) {
            ...
            $this->set_theme($config['is-child'], $config['parent-name'])->set_child_theme($config['stylesheet'], $config['theme-name'], $config['is-child']);
        }
        switch_theme($config['stylesheet']);
        $this->assertSame($this->format_the_html($html['expected']), $this->format_the_html(apply_filters('rocket_buffer', $html['original'])));
    }
}
