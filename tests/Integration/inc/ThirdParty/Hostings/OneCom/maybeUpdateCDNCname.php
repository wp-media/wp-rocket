...
class Test_MaybeUpdateCDNCname extends TestCase
{
    ...
    public function testShouldReturnExpected($config, $expected)
    {
        ...
        if ($config['onecom_performance_plugin_enabled']) {
            Functions\expect('rest_sanitize_boolean')->once()->andReturn($config['oc_cdn_enabled']);
            Functions\when('get_option')->alias(function ($value) use($config) {
        ...
        if ($config['oc_cdn_enabled'] && $config['onecom_performance_plugin_enabled']) {
            ...
            Functions\expect('wp_unslash')->times(2)->andReturn($domain_name, $http_host);
            Functions\expect('sanitize_text_field')->times(2)->andReturn($domain_name, $http_host);
        }
        $this->assertSame($expected['return'], apply_filters('pre_get_rocket_option_cdn_cnames', $config['cname']));
    }
