...
class Test_MaybeDeactivateRocketLazyload extends SmushSubscriberTestCase
{
    ...
    public function set_up()
    {
        global $wp_filter;
        parent::set_up();
        $this->filters = [$this->option_hook_prefix . 'lazyload' => null, $this->option_hook_prefix . 'lazyload_iframes' => null, 'update_option_wp_rocket_settings' => null];
    ...
    public function testShouldNotDisableWPRocketLazyLoad()
    {
        ...
        $this->setSmushSettings(false, []);
        $this->setRocketSettings(['lazyload' => 1, 'lazyload_iframes' => 1]);
        ...
        $this->setSmushSettings(true, ['jpeg' => true, 'iframe' => true]);
        $this->setRocketSettings(['lazyload' => 0, 'lazyload_iframes' => 0]);
        $this->subscriber->maybe_deactivate_rocket_lazyload();
        $new_settings = $this->getRocketLazyloadSettings();
    ...
    {
        $this->setSmushSettings(true, ['jpeg' => true, 'iframe' => false]);
        $this->setRocketSettings(['lazyload' => 1, 'lazyload_iframes' => 1]);
        $this->subscriber->maybe_deactivate_rocket_lazyload();
        $new_settings = $this->getRocketLazyloadSettings();
        $this->assertEmpty($new_settings['lazyload']);
    ...
    {
        $this->setSmushSettings(true, ['jpeg' => false, 'iframe' => true]);
        $this->setRocketSettings(['lazyload' => 1, 'lazyload_iframes' => 1]);
        $this->subscriber->maybe_deactivate_rocket_lazyload();
        $new_settings = $this->getRocketLazyloadSettings();
        $this->assertNotEmpty($new_settings['lazyload']);
    ...
    private function getRocketLazyloadSettings()
    {
        ...
        return ['lazyload' => isset($settings['lazyload']) ? $settings['lazyload'] : null, 'lazyload_iframes' => isset($settings['lazyload_iframes']) ? $settings['lazyload_iframes'] : null];
    }
}
