...
class Test_RocketAfterSaveOptions extends FilesystemTestCase
{
    ...
    protected static $transients = ['rocket_analytics_optin' => null];
    private $is_apache;
    private $hooks = [];
    private $expected;
    ...
    private function rocket_generate_advanced_cache_file($before_updating = false)
    {
        // Sets up the test before updating.
        if ($before_updating) {
            $this->dicontainer->addAdvancedCache($this->filesystem->getUrl($this->config['vfs_dir'] . 'plugins/wp-rocket/views/cache/'), $this->filesystem);
        ...
        // Checks after updating.
        if (isset($this->expected['rocket_generate_advanced_cache_file'])) {
            $this->assertSame($this->expected['rocket_generate_advanced_cache_file'], $this->filesystem->get_contents('vfs://public/wp-content/advanced-cache.php'));
        } else {
    ...
    private function rocket_generate_config_file()
    {
        if (isset($this->expected['rocket_generate_config_file'])) {
            $this->assertSame($this->expected['rocket_generate_config_file'], $this->filesystem->get_contents('vfs://public/wp-content/wp-rocket-config/example.org.php'));
        } else {
    ...
    {
        global $wp_filter;
        $hooks = ['pre_update_option_wp_rocket_settings', 'pre_update_option', 'default_option_wp_rocket_settings', 'update_option', 'update_option_wp_rocket_settings', 'updated_option'];
