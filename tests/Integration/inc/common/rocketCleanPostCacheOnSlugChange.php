...
class TestRocketCleanPostCacheOnSlugChange extends TestCase
{
    ...
    public function set_up()
    {
        ...
        $this->original_post = self::factory()->post->create_and_get(['post_title' => 'Some cool post', 'content' => 'Lorem ipsum dolor sit amet', 'post_status' => 'publish']);
    ...
    public function testShouldBailOutWhenPostStatusIsNotCorrect()
    {
        ...
        $post_id = edit_post([
            'post_status' => 'pending',
            // changed the post status from 'publish' to 'pending'.
            'post_ID' => $this->original_post->ID,
        ]);
        // Double-check the post status did change.
        $this->assertEquals('pending', get_post($post_id)->post_status);
        // makes sure the status changed.
    }
    /**
    ...
    public function testShouldBailOutWhenSlugHasntChanged()
    {
        ...
        $post_id = edit_post(['post_content' => 'Updated content happened here', 'post_ID' => $this->original_post->ID]);
        // Slug didn't change.
    ...
    public function testShouldFireRocketCleanFiles()
    {
        ...
        Functions\expect('rocket_clean_files')->once()->with(get_permalink($this->original_post->ID))->andReturn();
        // don't do anything.
        // Edit the post.
        $post_id = edit_post(['post_title' => 'Updated Cool Post', 'post_name' => 'updated-cool-post', 'post_status' => $this->original_post->post_status, 'post_ID' => $this->original_post->ID]);
