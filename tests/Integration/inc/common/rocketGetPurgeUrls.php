...
class Test_RocketGetPurgeUrls extends FilesystemTestCase
{
    ...
    private $site_options = ['stylesheet' => ''];
    ...
    private function create_posts($posts)
    {
        foreach ($posts as $post_id => $url) {
            ...
            $post_details = ['post_title' => !empty($post_name) ? $post_name : "home", 'post_type' => 20 !== $post_id ? 'post' : 'page'];
            if (4 === $post_id) {
    ...
    private function create_current_post($current_post_data)
    {
        ...
        $post_data = ['post_title' => $current_post_data['post_name'], 'post_name' => $current_post_data['post_name'], 'post_type' => $current_post_data['post_type'], 'post_author' => $this->authors_map[$current_post_data['post_author']]];
