...
class Test_maybeAddErrorNotice extends FilesystemTestCase
{
    ...
    public function testShouldDoAsExpected($config, $expected)
    {
        ...
        if ($expected['contains']) {
            $this->assertStringContainsString($this->format_the_html($expected['content']), $this->format_the_html($actual));
        } else {
            $this->assertStringNotContainsString($this->format_the_html($expected['content']), $this->format_the_html($actual));
        }
    }
}
