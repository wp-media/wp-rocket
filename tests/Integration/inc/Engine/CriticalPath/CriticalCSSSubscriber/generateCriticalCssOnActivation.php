...
class Test_GenerateCriticalCssOnActivation extends FilesystemTestCase
{
    ...
    protected static $transients = ['rocket_critical_css_generation_process_running' => null];
    ...
    public function set_up()
    {
        ...
        $this->did_filter = ['do_rocket_critical_css_generation' => 0];
    ...
    private function createSite($site_id)
    {
        if (empty(get_blogaddress_by_id($site_id))) {
            $this->factory->blog->create(['network_id' => $site_id, 'user_id' => self::$user_id]);
        }
    ...
    private function setCriticalCssPath($site_id)
    {
        ...
        $this->set_reflective_property(rocket_get_constant('WP_ROCKET_CRITICAL_CSS_PATH') . $site_id . '/', 'critical_css_path', $critical_css);
    }
