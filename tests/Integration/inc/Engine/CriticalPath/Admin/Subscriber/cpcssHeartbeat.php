...
class Test_CpcssHeartbeat extends AjaxTestCase
{
    ...
    public function set_up()
    {
        ...
        set_transient('rocket_critical_css_generation_process_running', ['generated' => 0, 'total' => 1, 'items' => []]);
    ...
    private function expectProcessGenerate($config, $expected)
    {
        ...
        $params = ['url' => $config['rocket_cpcss_generation_pending']['front_page.css']['url'], 'mobile' => $config['rocket_cpcss_generation_pending']['front_page.css']['mobile'], 'nofontface' => false];
        $job_id = 999;
        if (!empty($config['process_generate']['is_wp_error'])) {
            Functions\expect('wp_remote_post')->once()->with(APIClient::API_URL, ['body' => $params])->andReturn(new WP_Error('error', 'error_data'));
        } else {
            Functions\expect('wp_remote_post')->once()->with(APIClient::API_URL, ['body' => $params])->andReturn(['body' => '{"status":200,"success":true,"data":{"state":"generating","id":"' . $job_id . '"}}']);
            Functions\expect('wp_remote_get')->once()->with(APIClient::API_URL . "{$job_id}/")->andReturn(['body' => json_encode($config['process_generate'])]);
        }
    }
