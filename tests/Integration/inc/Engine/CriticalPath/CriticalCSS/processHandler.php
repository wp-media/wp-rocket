...
class Test_ProcessHandler extends FilesystemTestCase
{
    ...
    {
        parent::set_up();
        $this->to_be_removed = ['filters' => [], 'transients' => [], 'post_types' => [], 'posts' => [], 'taxonomies' => [], 'terms' => []];
        $this->expected_items = ['front_page' => ['type' => 'front_page', 'url' => 'http://example.org/', 'path' => 'front_page.css', 'check' => 0]];
    }
    ...
    public function testShouldHandleProcess($config, $expected)
    {
        ...
        $options = new Options_Data(['page_for_posts' => $page_for_posts, 'show_on_front' => $show_on_front]);
        $this->critical_css = new CriticalCSS($process, $options, $this->filesystem);
    ...
    private function addFilters($filters)
    {
        foreach ((array) $filters as $filter => $return) {
            ...
            $this->to_be_removed['filters'][] = [$filter => function () {
            }];
        }
    }
    ...
    private function prepareFrontPage($config)
    {
        if ('page' === $config['show_on_front'] && !empty($config['page_for_posts'])) {
            $this->expected_items['home'] = ['type' => 'home', 'url' => $config['page_for_posts_url'], 'path' => 'home.css', 'check' => 0];
        }
    }
    private function preparePosts($config)
    {
        ...
        foreach ((array) $config['post_types'] as $post_type) {
            if (!post_type_exists($post_type)) {
                ...
                $args = ['public' => true, 'publicly_queryable' => true, 'label' => $post_type];
        ...
        foreach ((array) $config['posts'] as $post) {
            $my_post = ['post_title' => 'Test title', 'post_content' => 'Test Content.', 'post_status' => $post->post_status];
            $post_id = wp_insert_post($my_post);
            $this->to_be_removed['posts'][] = $post_id;
            $this->expected_items[$post->post_type] = ['type' => $post->post_type, 'url' => get_permalink($post_id), 'path' => "{$post->post_type}.css", 'check' => 0];
        }
    }
    private function prepareTaxonomies($config)
    {
        ...
        foreach ((array) $config['terms'] as $term) {
            ...
            $this->expected_items[$term->taxonomy] = ['type' => $term->taxonomy, 'url' => get_term_link($term_array['term_id'], $term->taxonomy), 'path' => "{$term->taxonomy}.css", 'check' => 0];
        }
    }
    private function get_term($term)
    {
        ...
        $term_array = wp_insert_term('Test Term', $term->taxonomy);
