    ...
    public function testShouldDoExpected($config, $expected_file)
    {
        ...
        foreach ($config['expected_type'] as $expected_type) {
            if (!empty($expected_type['param'])) {
                Functions\expect($expected_type['type'])->once()->with($expected_type['param'])->andReturn($expected_type['return']);
            } else {
                Functions\expect($expected_type['type'])->once()->andReturn($expected_type['return']);
            }
        }
        ...
        if (!empty($expected_file)) {
            $this->assertSame($this->filesystem->getUrl($expected_file), $current_page_critical_css);
        } else {
            $this->assertSame($expected_file, $current_page_critical_css);
        }
    }
