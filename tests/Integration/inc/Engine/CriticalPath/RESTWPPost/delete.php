...
class Test_Delete extends RESTVfsTestCase
{
    ...
    {
        $this->post_id = !isset($config['post_data']['post_id']) ? $this->factory->post->create($config['post_data']) : $config['post_data']['post_id'];
        ...
        $this->files = ['dir' => $dirs, 'non_mobile' => "{$dirs}posts/{$this->post_type}-{$this->post_id}.css", 'mobile' => "{$dirs}posts/{$this->post_type}-{$this->post_id}-mobile.css"];
    ...
    {
        $this->assertSame($config['cpcss_exists_before'], $this->filesystem->exists($this->files['non_mobile']));
        if ($this->is_mobile) {
            $this->assertSame($config['mobile_cpcss_exists_before'], $this->filesystem->exists($this->files['mobile']));
        }
    }
    private function assertFilesDeleted($config)
    {
        $this->assertSame($config['cpcss_exists_after'], $this->filesystem->exists($this->files['non_mobile']));
        if ($this->is_mobile) {
            $this->assertSame($config['mobile_cpcss_exists_after'], $this->filesystem->exists($this->files['mobile']));
        }
    }
