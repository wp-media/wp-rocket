...
class Test_MaybePurgeCacheOnTermChange extends FilesystemTestCase
{
    ...
    {
        parent::set_up();
        register_taxonomy('not_public', 'post', ['public' => false, 'publicly_queryable' => false]);
    }
    ...
    public function testShouldPurgeWhenExpected($action, $is_public, $expected)
    {
        ...
        if ($is_public) {
            $term_config = ['name' => 'news', 'taxonomy' => 'category'];
        } else {
            $term_config = ['name' => 'test_term', 'taxonomy' => 'not_public'];
        }
