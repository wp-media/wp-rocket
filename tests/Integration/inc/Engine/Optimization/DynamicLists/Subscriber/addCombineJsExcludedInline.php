...
/**
 * Test class covering \WP_Rocket\Engine\Optimization\DynamicLists\Subscriber::add_combine_js_excluded_inline
 *
 * @group  DynamicLists
 */
class Test_AddCombineJsExcludedInline extends TestCase {
	...
	public function testShouldReturnExpected( $original, $list, $expected ) {
		...
		$this->assertSame(
			$expected,
			apply_filters( 'rocket_excluded_inline_js_content', $original )
		);
	}
}
