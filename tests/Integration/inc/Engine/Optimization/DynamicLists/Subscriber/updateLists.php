...
class Test_UpdateLists extends FilesystemTestCase
{
    ...
    public function testShouldDoExpected($user, $api_response, $expected)
    {
        ...
        $this->assertSame($expected['data'], $this->filesystem->get_contents($this->filesystem->getUrl('wp-content/wp-rocket-config/dynamic-lists.json')));
        $this->assertEquals($expected['transient'], get_transient('wpr_dynamic_lists'));
    }
