...
/**
 * Test class covering WP_Rocket\Engine\Admin\RocketInsights\Recommendations\Subscriber::clear_recommendations_on_lang_change
 *
 * @group RocketInsights
 * @group Recommendations
 * @group AdminOnly
 */
class Test_ClearRecommendationsOnLangChange extends TestCase {
	/**
	...
	public function testShouldDoAsExpected( $config, $expected ) {
		...
		if ( $expected['should_clear'] ) {
			// When clear is triggered, transient should be deleted (false).
			$this->assertFalse( $transient_after );
			return;
		}

		// When clear is not triggered, transient should remain unchanged.
