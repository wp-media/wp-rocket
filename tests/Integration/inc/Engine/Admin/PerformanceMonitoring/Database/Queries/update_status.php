...
/**
 * Test class covering \WP_Rocket\Engine\Admin\PerformanceMonitoring\Database\Queries\PerformanceMonitoring::update_status
 *
 * @group PerformanceMonitoring
 * @group AdminOnly
 */
class Test_UpdateStatus extends TestCase {
	use DBTrait;
	...
	public function testShouldWorkAsExpected( $config, $expected ) {
		...
		if ( ( ! isset( $config['uninstall_table'] ) || ! $config['uninstall_table'] ) && null !== $db_id && true === $result ) {
			...
			// Verify error_message was updated if provided.
			if ( ! empty( $config['error_message'] ) ) {
				$this->assertEquals( $config['error_message'], $updated_record->error_message );
			} elseif ( isset( $config['initial_record']['error_message'] ) ) {
				// If no error message provided, should keep existing one.
				$this->assertEquals( $config['initial_record']['error_message'], $updated_record->error_message );
			}
