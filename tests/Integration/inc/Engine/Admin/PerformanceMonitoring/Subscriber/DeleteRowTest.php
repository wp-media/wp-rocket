...
/**
 * Test class covering WP_Rocket\Engine\Admin\PerformanceMonitoring\Subscriber::delete_row
 *
...
class DeleteRowTest extends TestCase {
	...
	public function set_up() {
		...
		// Add a hook to capture when rocket_pm_job_deleted is fired
		add_action( 'rocket_pm_job_deleted', [ $this, 'capture_hook_fired' ] );
	...
	public function tear_down() {
		...
		remove_action( 'rocket_pm_job_deleted', [ $this, 'capture_hook_fired' ] );
	...
	private function setUpTest( $config ) {
		...
		if ( isset( $config['valid_nonce'] ) && $config['valid_nonce'] ) {
			$_GET['_wpnonce'] = \wp_create_nonce( 'delete_pm' );
		}
	...
	/**
	 * Callback to capture when rocket_pm_job_deleted hook is fired.
	 *
