...
**Preferred approach — emit events.** Do NOT post the comment directly with `gh`. Instead emit the events described below to the orchestrator's event stream; the orchestrator owns all GitHub writes. This keeps GitHub operations centralized, idempotent, and retryable.
...
Emit all events to `.TemporaryItems/Issues/wp-rocket/issue-<N>/orchestrator-events.jsonl` (where `<N>` is the issue number). Do NOT wait for a response to complete — emit and return immediately.

If verdict is PASS and there are no blockers, the summary event body is just:
...
```bash
# Legacy fallback — prefer emitting events.
# Only use this if the orchestrator event stream is unavailable.
