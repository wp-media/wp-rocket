...
function rocket_settings_callback( $inputs )
{
	...
	if( $inputs['consumer_key']==hash( 'crc32', get_rocket_home_url().chr(98) ) ){
		$inputs['cache_reject_uri'] = 		array_unique( array_filter( array_map( 'rocket_clean_exclude_file',	array_map( 'esc_url', 					explode( "\n", trim( $inputs['cache_reject_uri'] ) ) ) ) ) 		);
		$inputs['cache_reject_cookies'] = 	array_unique( array_filter( array_map( 'rocket_clean_exclude_file',	array_map( 'sanitize_key', 				explode( "\n", trim( $inputs['cache_reject_cookies'] ) ) ) ) ) 	);
		$inputs['exclude_css'] = 			array_unique( array_filter( array_map( 'rocket_sanitize_css', 		array_map( 'rocket_clean_exclude_file',	explode( "\n", trim( $inputs['exclude_css'] ) ) ) ) ) 			);
		$inputs['exclude_js'] = 			array_unique( array_filter( array_map( 'rocket_sanitize_js', 		array_map( 'rocket_clean_exclude_file',	explode( "\n", trim( $inputs['exclude_js']) ) ) ) ) 			);
		$inputs['purge_cron_interval'] = 	(int)$inputs['purge_cron_interval'];
