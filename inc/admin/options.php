...
{
	// Clean inputs
	if( $inputs['consumer_key']==hash( 'crc32', home_url( '/' ).chr(98) ) ){
		...
		$inputs['secret_key'] = 			@file_get_contents( WP_ROCKET_WEB_MAIN.WP_ROCKET_WEB_VALID . '?k='.sanitize_key( $inputs['consumer_key'] ).'&u='.urlencode( home_url( '/' ) ).'&v='.WP_ROCKET_VERSION );
