 ...
 * @return void
 */
function rocket_reload_cache_reject_uri( $old_value, $value ) {
	...
	foreach ( $diff as $path ) {
		...
		if ( ! (bool) preg_match( '#/[a-z0-9]+(?:-[a-z0-9]+)*(/|)$#', $path ) ) {
			...
			$wpdb->prepare( 'SELECT url FROM `' . $wpdb->prefix . 'wpr_rocket_cache` WHERE status = %s AND url REGEXP ' . $exp, 'completed' )
			);
...
add_action( 'update_option_' . rocket_get_constant( 'WP_ROCKET_SLUG' ), 'rocket_reload_cache_reject_uri', 10, 2 );
