 ...
 */
function rocket_deprecated_class( $class, $version, $replacement = null, $parent_class = '' ) {
	 ...
	 */
	do_action( 'rocket_deprecated_class_run', $class, $version, $replacement, $parent_class );
		...
		if ( ! empty( $parent_class ) ) {
			...
			if ( ! empty( $replacement ) ) {
				...
				call_user_func(
					...
					sprintf(
						...
						__( 'The called class %1$s extending %2$s is <strong>deprecated</strong> since version %3$s! Use %4$s instead.', 'rocket' ),
						'<code>' . $class . '</code>',
			...
			call_user_func(
				...
				sprintf(
					...
					__( 'The called class %1$s extending %2$s is <strong>deprecated</strong> since version %3$s!', 'rocket' ),
					'<code>' . $class . '</code>',
					'<code>' . $parent_class . '</code>',
					'<strong>' . $version . '</strong>'
				)
