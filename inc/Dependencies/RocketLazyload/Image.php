...
class Image {
	...
	public function lazyloadBackgroundImages( $html, $buffer ) {
		if ( ! preg_match_all( '#<(?<tag>div|figure|section|span|li|a|aside)\s+(?<before>[^>]+[\'"\s])?style\s*=\s*([\'"])(?<styles>.*?)\3(?<after>[^>]*)>#is', $buffer, $elements, PREG_SET_ORDER ) ) {
			return $html;
		}
	...
	private function addLazyClass( $element ) {
		...
		if ( ! $class ) {
			$result = preg_replace( '#<(img|div|figure|section|li|span|a|aside)([^>]*)>#is', '<\1 class="rocket-lazyload"\2>', $element );
