...
class Rest extends WP_REST_Controller {
	...
	protected function get_url_validation_payload( string $url ): array {
		...
		if ( 'local' === wp_get_environment_type() ) {
			$payload['error']   = true;
			$payload['message'] = __( 'Performance monitoring is disabled for local environments.', 'rocket' );
		...
		if ( ! $this->context->is_allowed() ) {
			$payload['error']   = true;
			$payload['message'] = __( 'Performance monitoring is disabled.', 'rocket' );
		...
		if ( ! $response ) {
			$payload['error']   = true;
			$payload['message'] = __( 'This URL could not be reached. Please make sure it is a publicly accessible page.', 'rocket' );
