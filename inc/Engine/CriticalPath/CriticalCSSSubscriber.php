...
JS;
	...
	public function async_css( $buffer ) {
		...
		foreach ( $tags_match[0] as $i => $tag ) {
			...
			$onload  = preg_replace( '~' . preg_quote( $tags_match[3][ $i ], '~' ) . '~iU', ' data-rocket-async="style" as="style" onload="" onerror="this.removeAttr(\'data-rocket-async\')"' . $tags_match[3][ $i ] . '>', $tags_match[3][ $i ] );
