...
class AuthorizeCallback {
	...
	public function handle_request(): void {
		...
		if ( false === $state_data || ! is_array( $state_data ) ) {
			McpLogger::log( 'CALLBACK', 'rejected: state transient not found or expired', array( 'state' => $state ) );
			wp_die( esc_html__( 'Invalid or expired state. Please restart the authorization flow.', 'wp-rocket' ), esc_html__( 'OAuth Error', 'wp-rocket' ), array( 'response' => 400 ) );
		}
	...
	private function render_consent_screen( string $state, array $client ): void {
		...
		<body>
			<div class="consent-card">
				<h1><?php esc_html_e( 'Authorize access to your site?', 'wp-rocket' ); ?></h1>
				...
				<div class="client-block">
					...
					<?php if ( '' !== $client_id ) : ?>
						<div class="client-url">
							<?php esc_html_e( 'ID:', 'wp-rocket' ); ?>
							<a href="<?php echo $client_id; // Already esc_url'd above. ?>" rel="noopener noreferrer" target="_blank"><?php echo $client_id; ?></a>
						</div>
					<?php endif; ?>
				...
				<form method="post" action="<?php echo $consent_url; ?>">
