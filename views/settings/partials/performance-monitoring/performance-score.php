...
<div class="wpr-percentage-indicator">
	...
	<?php elseif ( isset( $data['status'] ) && 'failed' === $data['status'] ) : ?>
		<div class="wpr-percentage-circle  status-red <?php echo isset( $data['is_blurred'] ) && $data['is_blurred'] ? 'blurred' : ''; ?>">
			!
		</div>
	<?php else : ?>
		<div class="wpr-percentage-circle <?php echo esc_html( $data['status-color'] ?? '' ); ?> <?php echo isset( $data['is_blurred'] ) && $data['is_blurred'] ? 'blurred' : ''; ?>">
			<?php echo esc_html( $data['score'] ); ?>
		</div>
	<?php endif; ?>
	<?php if ( ( isset( $data['status'] ) && 'failed' === $data['status'] ) || ( isset( $data['is_blurred'] ) && $data['is_blurred'] ) ) : ?>
