...
$is_allowed_cookie = preg_match( '/({{COOKIES_NOT_CACHED}})/', var_export( $_COOKIE , true ) ) ? false : true;
...
if( isset( $_SERVER['HTTP_USER_AGENT'] ) && $_SERVER['REQUEST_METHOD'] == 'GET' && empty( $_GET ) && $is_not_feed && $is_allowed_cookie )
{
	...
	if( strstr( $http_response_header[0], '200' )!=false )
	{
    	...
    	if( !is_dir( '{{CACHE_DIR}}' . $_SERVER['REQUEST_URI'] ) )
    		mkdir( '{{CACHE_DIR}}' . $_SERVER['REQUEST_URI'], 0755, true );


    	// Create file cache
		file_put_contents( '{{CACHE_DIR}}' . $_SERVER['REQUEST_URI'] . '/index.html.gz', gzencode( $data, 6 ) );
		...
		readfile( '{{CACHE_DIR}}' . $_SERVER['REQUEST_URI'] . '/index.html.gz' );
		exit;
